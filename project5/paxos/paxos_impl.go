...
func (gc *GarbageCollector) getDone(p int) int {
	return int(atomic.LoadInt64(&(gc.LastDone[p])))
}

func (gc *GarbageCollector) setDone(args DoneArgs) {
	var l = gc.getDone(args.Source)
	if l < args.LastDone {
		gc.LastDone[args.Source] = int64(args.LastDone)
		gc.Updates++
	}
}

func (gc *GarbageCollector) updateBound() int {
	var result = gc.getDone(0)
	for i := range gc.LastDone {
		var done = gc.getDone(i)
		if done < result {
			result = done
		}
	}
...
func (px *Paxos) consensus(votes uint) bool {
...
func (px *Paxos) getDone(p int) int {
...
func (px *Paxos) collectGarbage() {
	var result = int64(px.impl.GCState.updateBound())
...
func (px *Paxos) Start(seq int, v interface{}) {
	go func() {
		...
		var statistics = PrepareBroadcast{
			Consensus: false,
			Decided:   false,
			NextID:    px.initialProposal(),
			Proposal:  log.Instance.Accepted,
		}
		for !px.isdead() && log.Fate == Pending {
			var proposal = Proposal{statistics.NextID, v}
			var prepare_args = PrepareArgs{
				Instance: seq,
				Proposal: statistics.NextID,
			}
...
func (px *Paxos) Min() int {
	...
	defer px.mu.Unlock()
	px.collectGarbage()
	return int(px.impl.LogRange.Lower)
}
