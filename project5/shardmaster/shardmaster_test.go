...
func TestBasic(t *testing.T) {
	...
	for i := 0; i < len(cfa); i++ {
		...
		if len(c.Groups) != len(cfa[i].Groups) {
			println("	Expect len(gp)", len(cfa[i].Groups), ",got", len(c.Groups), "servers")
		...
		for gid, sa := range c.Groups {
			...
			if ok == false || len(sa1) != len(sa) {
				if ok == false {
					println("	Error: group", gid, "not existed in config", i)
				}
				println("	Expect group", gid, "has", len(sa), "servers, got", len(sa1), "servers")
...
func TestRepeatedJoin(t *testing.T) {
	...
	//// Initialize Shardmaster Paxos Servers /////
	for i := 0; i < nservers; i++ {
		kvh[i] = port("basic", i)
	}
	for i := 0; i < nservers; i++ {
	...
	fmt.Printf("[TEST] Join gid 1 ...\n")
	var gid1 int64 = 1
	ck.Join(gid1, []string{"x", "y", "z"})
	check(t, []int64{gid1}, ck)
	cfa[1] = ck.Query(-1)

	fmt.Printf("[TEST] Join gid 2 ...\n")
	var gid2 int64 = 2
	ck.Join(gid2, []string{"a", "b", "c"})
	check(t, []int64{gid1, gid2}, ck)
	cfa[2] = ck.Query(-1)
	...
	fmt.Printf("[TEST] Leave gid 1 ...\n")
	ck.Leave(gid1)
	check(t, []int64{gid2}, ck)
	cfa[4] = ck.Query(-1)

	fmt.Printf("[TEST] Re-join gid 1 ...\n")
	var gid4 int64 = 1
	ck.Join(gid1, []string{"x", "y", "z"})
	check(t, []int64{gid2, gid4}, ck)
	cfa[5] = ck.Query(-1)

	fmt.Printf("[TEST] Leave gid 2 ...\n")
	ck.Leave(gid2)
	check(t, []int64{gid4}, ck)
	cfa[6] = ck.Query(-1)

	fmt.Printf("[TEST] Re-join gid 2 ...\n")
	var gid5 int64 = 2
	ck.Join(gid5, []string{"a", "b", "c"})
	check(t, []int64{gid4, gid5}, ck)
	cfa[7] = ck.Query(-1)
